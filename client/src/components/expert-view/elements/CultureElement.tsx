...
interface CultureElementProps {
  componentType: ComponentType;
  data: ElementsExpertData;
  onChange: (next: ElementsExpertData) => void;
  /** Expert data from the school-wide (`overall`) component — used for ring “same as school-wide” mirrors. */
...
export function CultureElement({ componentType, data, onChange, schoolWideElementsExpertData }: CultureElementProps) {
  ...
  function handleBucketChange(questionId: string, bucketId: string, value: BucketValue) {
    ...
    onChange({ ...data, culture: nextElementData });
  }

  function handleSharedBucketChange(syncedBucketId: string, value: BucketValue) {
    onChange({ ...data, __shared__: { ...sharedData, [syncedBucketId]: value } });
  }
  ...
  const visibleQuestions = CULTURE_ELEMENT.questions.filter((q) => q.section === activeSection);
