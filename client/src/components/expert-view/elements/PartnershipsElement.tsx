...
interface PartnershipsElementProps {
...
export function PartnershipsElement({ componentType, data, onChange }: PartnershipsElementProps) {
  const [activeSection, setActiveSection] = useState<ElementSection>('practices');
  const [openQuestionId, setOpenQuestionId] = useState<string | null>(null);
  const elementData = data['partnerships'] ?? {};
  ...
  function handleBucketChange(questionId: string, bucketId: string, value: BucketValue) {
    ...
    onChange({ ...data, partnerships: { ...elementData, [key]: value } });
  }
  ...
  const visibleQuestions = PARTNERSHIPS_ELEMENT.questions.filter((q) => q.section === activeSection);
